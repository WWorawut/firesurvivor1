...
{!this.props.fireroom ?
 ...
 null
      ...
      {this.Redirect()}
      {this.props.fireroom === false ?
        <Entity events={{click:this.openpop({link:'link',state:'popup',sound:sound1}) , mouseenter:this.mouseenter('scale1') , mouseleave:this.mouseleave('scale1')}}  primitive='a-image' material={{ src: choosebutton}} scale={{x: this.state.scale1, y: this.state.scale1, z:this.state.scale1}} rotation={{x: 0, y: -90 ,z: 0}} position={{x:15, y: -1, z: -3}}/>
        :null}
...
export default connect(connectscore)(Mfire);
