...
import bu from "../picture2/popscore/popfirestair.png";
...
class stair3600 extends React.Component {
  ...
  Redirect=()=>{
    if(this.state.A){ return <Redirect to="/officeoutside"/> }
    if(this.state.up){ return <Redirect to="/deck"/> }
    if(this.state.down){ return <Redirect to="/stair3601"/> }
}
  ...
  render() {
    ...
    <div>
      ...
      <Entity events={{click:this.next('A') , mouseenter:this.mouseenter('scale1') , mouseleave:this.mouseleave('scale1')}}  primitive='a-image' material={{ src: gobutton}} scale={{x: this.state.scale1, y: this.state.scale1, z:this.state.scale1}} rotation={{x: 0, y: 0 ,z: 0}} position={{x:3, y: -1, z: -10}}/>
        ...
        <Entity events={{click:this.openpop({link:'down',score:25,state:'popup',sound:sound3}) , mouseenter:this.mouseenter('scale2') , mouseleave:this.mouseleave('scale2')}}  primitive='a-image' material={{ src: gobutton}} scale={{x: this.state.scale2, y: this.state.scale2, z:this.state.scale2}} rotation={{x: 0, y: 0 ,z: 0}} position={{x:-3, y: 0, z:12}}/> 
...
export default connect(connectscore)(stair3600);
